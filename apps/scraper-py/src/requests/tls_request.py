...
class LazyTLSRequest(TLSRequest):
    ...
    def __init__(self, *args, **kwargs):
        executor: ThreadPoolExecutor | None = kwargs.pop("executor", None)
        super().__init__(*args, **kwargs)

        self._thread: Thread | Future
        if executor:
...
) -> Iterable[Response | LazyTLSRequest]:
    """
...
@overload
def request(method: str, url: str, *args, **kwargs) -> Response | LazyTLSRequest: ...


def request(method: str, url: str | Iterable[str], *args, **kwargs):
    """
...
def send(r, pool: Pool | None = None):
    """
...
def map(
    ...
    for inc in range(0, len(requests), size):
        ...
        try:
            resps: list[Response | None] = response.ProcessResponsePool(
                processed_reqs
            ).execute_pool()
        except Exception as e:
