...
class FileUtils:
    ...
    @staticmethod
    def to_items_list(value: dict | tuple | list) -> list[tuple[str, Any]]:
        if isinstance(value, Mapping):
            value = value.items()
        return list(value)

    @staticmethod
    def _guess_filename(obj) -> str | None:
    ...
    @staticmethod
    def get_fields(data: dict | tuple) -> Iterator[tuple[str, bytes]]:
    ...
    @staticmethod
    def encode_files(
        files: dict[str, BufferedReader | tuple], data: dict | tuple | None = None
    ) -> tuple[bytes, str]:
        """
...
class File:
    file_name: str
    file_obj: BufferedReader | str | bytes | bytearray
