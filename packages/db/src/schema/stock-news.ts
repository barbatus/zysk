...
export const stockNewsTable = mySchema.table(
  "stock_news",
  {
    ...
    mainSymbol: varchar("main_symbol", { length: 80 }),
    extractedSymbols: jsonb("extracted_symbols").$type<string[]>().default([]),
  ...
  (t) => ({
    ...
    extractedSymbolsIdx: index("extracted_symbols_gin_idx").using(
      "gin",
      sql`extracted_symbols jsonb_path_ops`,
    ),
    mainSymbolIdx: index("main_symbol_idx").on(t.mainSymbol),
    urlIdx: uniqueIndex("url_idx").on(t.url),
